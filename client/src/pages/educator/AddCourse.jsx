    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit()} className='flex flex-col gap-4 max-w-md w-full text-gray-500 '>
        ...
        <div className='flex flex-col gap-1'> 
          ...
          placeholder='0' min={0} max={100} className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required name="" id="" />
        </div>

        <div>
          {chapters.map((chapter,chapterIndex)=>(
            <div key={chapterIndex} className='bg-white border
             rounded-lg mb-4'>
              ...
              {!chapter.collapsed && (
                ...
                {chapter.chapterContent.map((lecture , lectureIndex)=>(
                  <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                    <span>{lectureIndex + 1} {lecture.lectureIndex} - {lecture.
                  ...
                  cursor-pointer mt-2' onClick={()=>handleLecture('add',chapter.chapterId)} >+ Add Lectures</div>
                </div>
              )}
